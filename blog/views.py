...
def index(request):
  ...
  today_created_blogs = Blog.objects.all().filter(is_public='公開', created__date=today)
  yes_created_blogs = Blog.objects.all().filter(is_public='公開', created__date=yesterday)
  today_updated_blogs =  Blog.objects.all().filter(is_public='公開', updated__date=today)
  yes_updated_blogs =  Blog.objects.all().filter(is_public='公開', updated__date=yesterday)

  count = Blog.objects.order_by('-updated').filter(is_public='公開', created__date=today).count()
...
class BlogList(LoginRequiredMixin, ListView):
  ...
  def get_context_data(self, **kwargs):
    ...
    yesterday = today - timedelta(days=1)
    context['today_created_blogs'] = Blog.objects.all().filter(is_public='公開', created__date=today)
    context['yes_created_blogs'] = Blog.objects.all().filter(is_public='公開', created__date=yesterday)
    context['today_updated_blogs'] =  Blog.objects.all().filter(is_public='公開', updated__date=today)
    context['yes_updated_blogs'] =  Blog.objects.all().filter(is_public='公開', updated__date=yesterday)
...
def my_blogs(request):
  ...
  blogs = Blog.objects.order_by('-updated').filter(user=request.user, is_public='公開')
...
@login_required
def drafts(request):
  user_drafts = Blog.objects.order_by('-created').filter(user=request.user, is_public='非公開')
  context = {
    # 'user': user,
...
class BlogCreate(LoginRequiredMixin, CreateView):
  ...
  fields = ['title', 'content_1', 'content_2', 'content_3', 'link_1', 'link_2', 'tags', 'is_public']
